#!/bin/zsh
# The fine alpha_lo ladder: does any rung floor the sea chart's (coherent) sea and not
# config_stability's (coherent) sponge? alpha_lo = 0.02 / 0.005 / 0.001 floor only where the
# children's structured fill exceeds 97.3% / 99.3% / 99.86% of the box.
...
mkdir -p $ROOT/output
for tgt in config_stability preset_shape_h1; do
...
echo "phase3_fine done $(date +%H:%M:%S)"
